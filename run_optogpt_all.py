...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', default=512, type=int, help='Batch size (suggest 128/256)')
    parser.add_argument('--dropout', default=0.05, type=float, help='dropout rate')
    parser.add_argument('--max_lr', default=5e-4, type=float, help='NoamOpt factor (suggest 5e-4~1e-3)')
    parser.add_argument('--warm_steps', default=16000, type=int, help='NoamOpt warmup steps (suggest 5k~12k)')

    parser.add_argument('--smoothing', default=0.05, type=float, help='Label smoothing for KL')

    # ===== Model =====
    parser.add_argument('--layers', default=6, type=int, help='Decoder layers (DBR suggest 2)')
