...
- 【新增】在均值曲线图上写入“代表曲线差异的指标”（Oracle vs GT / Pred vs GT）
- 【新增】支持画“单个样例”的曲线（GT/Oracle/Pred）：
    --plot_single --plot_single_k K 或 --plot_single_ids "0,5,12"
...
def calc_spec_tmmfast_batch(
    ...
    # NOTE: some tmm_fast builds use (pol,n,d,lambda,theta). If you hit shape asserts, swap args.
...
def _curve_metrics(y_hat: np.ndarray, y_gt: np.ndarray, x: np.ndarray) -> Dict[str, float]:
    ...
    y_gt  = np.asarray(y_gt, dtype=np.float64).reshape(-1)
...
def _plot_curve_and_err(
    ...
    x = np.asarray(wavelengths_um, dtype=np.float64).reshape(-1)
    gt = np.asarray(gt_mean, dtype=np.float64).reshape(-1)
    oc = np.asarray(oracle_mean, dtype=np.float64).reshape(-1)
    pr = np.asarray(pred_mean, dtype=np.float64).reshape(-1)  # NOTE: pr not pd !!

    m_oracle = _curve_metrics(oc, gt, x)
    m_pred   = _curve_metrics(pr, gt, x)
    ...
    plt.plot(x, pr, label="Pred(TMM(pred))")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, f"mean_curve__{group_name}.png"), dpi=200)
    plt.close()

    # abs error curves
    plt.figure()
    plt.plot(x, np.abs(oc - gt), label="|Oracle-GT|")
    plt.plot(x, np.abs(pr - gt), label="|Pred-GT|")
    plt.xlabel("Wavelength (um)")
    plt.ylabel(f"|Δ| ({ylabel})")
    plt.title(f"Mean Abs Error - {group_name}")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, f"mean_abs_err__{group_name}.png"), dpi=200)
    plt.close()
    ...
    if save_signed_err:
        ...
        plt.plot(x, (pr - gt), label="Pred-GT")
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, f"mean_signed_err__{group_name}.png"), dpi=200)
        plt.close()
    ...
    if save_csv:
        df = pd.DataFrame({
            ...
            "abs_err_pred": np.abs(pr - gt),
            "signed_err_oracle": (oc - gt),
            "signed_err_pred": (pr - gt),
        })
        df.to_csv(os.path.join(out_dir, f"mean_curve__{group_name}.csv"), index=False)

    with open(os.path.join(out_dir, f"metrics__{group_name}.json"), "w", encoding="utf-8") as f:
        json.dump({"oracle_vs_gt": m_oracle, "pred_vs_gt": m_pred}, f, ensure_ascii=False, indent=2)

def _plot_groups_mean_curves(
    wavelengths_um: np.ndarray,
    gt_spec_arr: np.ndarray,
    oracle_spec_arr: np.ndarray,
    pred_spec_arr: np.ndarray,
    group_keys: List[str],
    out_dir: str,
    group_prefix: str,
    plot_min_count: int,
    ...
    for g, ids in groups_sorted:
        ...
        if st in ("R", "T"):
            if gt_mean.shape[0] != wavelengths_um.shape[0]:
                raise ValueError(f"[Plot] spec_dim={gt_mean.shape[0]} but W={wavelengths_um.shape[0]} for spec_type={st}")
            _plot_curve_and_err(
...
def _plot_single_sample_curves(
    wavelengths_um: np.ndarray,
    gt: np.ndarray,
    oracle: np.ndarray,
    pred: np.ndarray,
    ...
    oc = np.asarray(oracle, dtype=np.float64).reshape(-1)
    pr = np.asarray(pred, dtype=np.float64).reshape(-1)
    ...
    if st in ("R", "T"):
        ...
        m_pred = _curve_metrics(pr, gt, x)
        txt = (
            f"Oracle MAE={m_oracle['MAE']:.4g}, RMSE={m_oracle['RMSE']:.4g}, MAX={m_oracle['MAX']:.4g}\n"
            f"Pred   MAE={m_pred['MAE']:.4g}, RMSE={m_pred['RMSE']:.4g}, MAX={m_pred['MAX']:.4g}"
        )
        ...
        plt.xlabel("Wavelength (um)")
        plt.ylabel(st)
        plt.title(f"Single Sample - {name} ({st})")
        plt.legend()
        plt.gca().text(
            0.99, 0.99, txt,
            transform=plt.gca().transAxes,
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, f"single__{name}.png"), dpi=200)
        plt.close()
        ...
        plt.xlabel("Wavelength (um)")
        plt.ylabel(f"|Δ| ({st})")
        plt.title(f"Single Sample Abs Error - {name} ({st})")
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, f"single_abs_err__{name}.png"), dpi=200)
        plt.close()
        ...
        if save_csv:
            ...
            df.to_csv(os.path.join(out_dir, f"single__{name}.csv"), index=False)

        with open(os.path.join(out_dir, f"single_metrics__{name}.json"), "w", encoding="utf-8") as f:
            json.dump({"oracle_vs_gt": m_oracle, "pred_vs_gt": m_pred}, f, ensure_ascii=False, indent=2)

    elif st == "R_T":
        W = x.shape[0]
        if gt.shape[0] != 2 * W:
            raise ValueError(f"spec_type=R_T expects D=2W. Got D={gt.shape[0]}, W={W}")

        _plot_single_sample_curves(x, gt[:W], oc[:W], pr[:W], out_dir, name + "__R", "R", save_csv)
        _plot_single_sample_curves(x, gt[W:], oc[W:], pr[W:], out_dir, name + "__T", "T", save_csv)
    else:
        raise ValueError(f"Unknown spec_type: {spec_type}")
...
def main():
    ...
    # ===== single sample plot options =====
    ap.add_argument("--plot_single", action="store_true", help="Plot single-sample curves (GT/Oracle/Pred).")
    ap.add_argument("--plot_single_k", type=int, default=20, help="How many single samples to plot.")
    ap.add_argument("--plot_single_seed", type=int, default=0, help="Seed for sampling single examples.")
    ap.add_argument("--plot_single_ids", type=str, default="", help="Comma-separated eval-order indices, e.g. '0,5,12'.")
    ...
    if spec_dim != cfg.spec_dim:
        raise ValueError(
            f"spec_dim mismatch after slicing: dev_spec={spec_dim}, ckpt expects={cfg.spec_dim}. "
    ...
    # curve arrays
    gt_spec_arr = np.asarray(gt_spec_list, dtype=np.float32)
    oracle_spec_arr = np.asarray(oracle_spec_list, dtype=np.float32)
    pred_spec_arr = np.asarray(pred_spec_list, dtype=np.float32)
    ...
    if args.plot_mean_by_type or args.plot_mean_by_layers:
        ...
        if args.plot_mean_by_type:
            _plot_groups_mean_curves(
                ...
                out_dir=os.path.join(args.out_dir, "by_type"),
                group_prefix="type",
        ...
        if args.plot_mean_by_layers:
            ...
            _plot_groups_mean_curves(
                ...
                out_dir=os.path.join(args.out_dir, "by_layers_bucket"),
                group_prefix="layers",
                plot_min_count=args.plot_min_count,
                plot_max_groups=args.plot_max_groups,
                save_csv=args.save_curve_csv,
    ...
    # single sample plots
    if args.plot_single:
        out_single = os.path.join(args.out_dir, "single_samples")
        os.makedirs(out_single, exist_ok=True)
        N_eval = gt_spec_arr.shape[0]

        chosen = []
        if args.plot_single_ids.strip():
            for s in args.plot_single_ids.split(","):
                s = s.strip()
                if not s:
                    continue
                j = int(s)
        ...
        if not chosen:
            rng = np.random.default_rng(args.plot_single_seed)
            k = min(max(1, args.plot_single_k), N_eval)
            chosen = rng.choice(np.arange(N_eval), size=k, replace=False).tolist()

        print(f"[SinglePlot] plotting {len(chosen)} samples -> {out_single}")
        for j in chosen:
            ii = idxs[j]
            t = type_list[j]
            nl = layers_list[j]
            name = _sanitize_name(f"j{j}_dev{ii}_type{t}_L{nl}")
            _plot_single_sample_curves(
                wavelengths_um=wavelengths_um,
                gt=gt_spec_arr[j],
                oracle=oracle_spec_arr[j],
                pred=pred_spec_arr[j],
                out_dir=out_single,
                name=name,
                spec_type=args.spec_type,
                save_csv=args.save_curve_csv,
...
1) 画单样例（随机 50 个）：
python eval_optogpt_universal_mae_tmmfast_full.py \
...
2) 指定画 eval 顺序的第 0, 5, 12 个样例：
python eval_optogpt_universal_mae_tmmfast_full.py \
  ... \
  --plot_single \
  --plot_single_ids "0,5,12"
"""
