...
"""
generate_mix_optogpt_style_v2_tmm_fast_batch.py

A curriculum-friendly OptoGPT-style mixed dataset generator.

Key fixes vs your v1:
1) Boundary condition unified: air -> stack -> Glass_Substrate (exit), and force substrate k=0 (real(n)).
2) Mixture is more learnable: fixed structures dominate; random reduced and simplified (dielectrics only, shorter stacks).
3) Stratified split into train/dev/test with identical type ratios.
4) Token system unified: Material_ThicknessNm with thickness quantized to THK_STEP_NM grid.
...
Usage:
python generate_mix_optogpt_style_v2_tmm_fast_batch.py
"""
...
# 0) Global config
# =========================================================
NUM_SAMPLES = 200000          # <-- try 200k first; scale to 1_000_000 later
GLOBAL_SEED = 42
SPLIT_SEED = 42
...
# thickness quantization (mentor asked "interval")
THK_MIN_NM = 10
THK_MAX_NM = 500
THK_STEP_NM = 10

# ---- Mixture: more learnable start ----
RATIO_FIXED  = 0.75     # fixed structures dominate
RATIO_RANDOM = 0.25
...
# ---- Random stacks: simplified for stage-1 ----
RANDOM_MIN_LAYERS = 2
RANDOM_MAX_LAYERS = 10          # <-- key: keep short to reduce multi-solution blowup
RANDOM_P_SWITCH_MAT = 0.70

# ---- Unified optics environment ----
POL = "s"
INC_MEDIUM = "air"
...
# NOTE: FP includes lossy materials; keep it but do NOT include these in RANDOM during stage-1
FP_METALS = ["Ag", "Al", "TiN"]
FP_CAVITY_MATS = ["SiO2", "ITO"]
FP_METAL_THK_SET_NM = [20, 30, 40]
FP_ITO_THK_SET_NM   = [120, 200, 400]

# ---- Random material pool (stage-1): dielectrics only ----
RANDOM_MATERIAL_POOL = [
    "MgF2", "SiO2", "Si3N4",
    "TiO2", "Ta2O5", "HfO2", "Nb2O5", "AlN",
]

# Center wavelength options for "rule" structures
LAMBDA0_SET_UM = [1.05, 1.31, 1.55]

# IO
NK_DIR = "./dataset/data"
OUT_DIR = "./dataset/mix_optogpt_style_v2"
...
# =========================================================
# 2) Fixed structures: DBR / AR / FP (quantized thickness)
# =========================================================
...
def gen_ar(nk_dict: Dict[str, torch.Tensor]) -> Tuple[List[str], List[int], Dict[str, Any]]:
    ...
    if family == "1L":
        ...
        seq = [Hm, Lm] if random.random() < 0.5 else [Lm, Hm]
    elif family == "3L":
...
def gen_fp(nk_dict: Dict[str, torch.Tensor]) -> Tuple[List[str], List[int], Dict[str, Any]]:
    ...
    meta = dict(type="FP", family="hybrid", mirror=f"{H}/{L}", pairs=int(pairs), lambda0_um=float(lambda0_um),
                cavity=cav, cav_mode=cav_mode, metal=metal, num_layers=int(len(mats)))
    return mats, thks, meta


# =========================================================
# 3) Random stacks (stage-1: dielectrics only)
# =========================================================
...
def stratified_split_indices(meta_list: List[Dict[str, Any]], seed: int):
    ...
    for t, ids in type2idx.items():
        ...
        # ensure sum <= n, give remainder to test
...
def main():
    ...
    # Needed materials for nk (all fixed + random + substrate)
    ...
    pbar = tqdm(total=NUM_SAMPLES, desc="Generating MIX v2 dataset (tmm_fast batch)")
