# # # dbr_dataset_generator_v3_verified.py
# # # 在 v2 的基础上：把“DBR 结构语法检查 + 光谱行为检查”整合进生成器
# # # 输出：
# # # - dbr_dataset.csv / .pkl：包含结构、meta、QA指标、R/T列
# # # - train_structure.pkl：list[list[str]]
# # # - train_spectrum.pkl：list[list[float]]，每条为 [R..., T...] 拼接（支持 R / T / R_T 训练切分）
# # #
# # # 你只需要：
# # # 1) 把你那张表保存成 ./dbr_exploratory_pairs.csv（含表头 H,L,dn_med,cross_rate,k_max）
# # # 2) NK_DIR 目录下有对应材料的 csv（wl,n,k）
# # #
# # # 说明：
# # # - 全量“逐条做 TMM 光谱检查”会慢，所以提供两种模式：
# # #   A) VERIFY_MODE='all'    : 每条都做结构+光谱验证（最严格，最慢）
# # #   B) VERIFY_MODE='sample' : 每条做结构检查；光谱检查按概率抽检（推荐大规模生成）
# # #
# # # 你现在 6000 条测试：建议 VERIFY_MODE='all'
...
# # PAIR_CSV = './dbr_exploratory_pairs.csv'
# # OUT_DIR = './output'
# # os.makedirs(OUT_DIR, exist_ok=True)

# # SANITY_PLOT_NUM = 2  # 生成前画几条（0关掉）

# # # ===== 验证策略 =====
# # VERIFY_MODE = 'all'       # 'all' or 'sample'
# # SPECTRUM_CHECK_PROB = 0.05  # VERIFY_MODE='sample' 时，抽检比例（比如 5%）
...
# # # 3) DBR 验证阈值（可按需求调）
# # # =========================
# # # 结构层：λ/4 厚度偏差阈值（你的扰动最大 0.1，给 0.25 非常安全）
# # MAX_REL_ERR_QW = 0.25

# # # 光谱层：stop-band 判定阈值（可调）
# # PEAK_MIN = 0.85
# # BAND_THRESHOLD = 0.80
# # BAND_MIN_WIDTH_NM = 50  # stop-band 最小宽度
...
# #     cross_rate: float
# #     k_max: float
# #     source: str  # 'industry' or 'explore'
# #     tier: str    # 'industry'/'strong'/'mid'/'weak'
...
# #     # 补齐 industry 白名单（CSV 里没有也保留）
# #     existing_industry = set((p.H, p.L) for p in buckets['industry'])
# #     for (H, L) in INDUSTRY_CORE:
# #         if (H, L) not in existing_industry:
# #             buckets['industry'].append(PairInfo(H, L, dn_med=-1.0, cross_rate=0.0, k_max=-1.0,
# #                                                source='industry', tier='industry'))
...
# #     # 2) stop-band：阈值法找包含 lambda0 的连续高反射段
...
# # # sanity plot
# # # =========================
# # def visualize_one(nk_dict, buckets):
...
# #     plt.ylabel("Value")
# #     plt.title(f"DBR | {meta['pair_tier']} | {meta['H']}/{meta['L']} | {meta['pairs']} pairs | {meta['mode']} | ok={ok_s and ok_r}")
# #     plt.grid(alpha=0.3)
# #     plt.legend()
# #     plt.tight_layout()
# #     plt.show()
...
# #     print("Meta:", meta)
# #     print("Structure check:", ok_s, info_s)
# #     print("Spectrum check :", ok_r, info_r)
...
# #     pbar = tqdm(total=NUM_SAMPLES, desc="Generating verified DBR")
...
# #                 max_rel_err=MAX_REL_ERR_QW
# #             )
# #             if not ok_s:
# #                 fail_struct += 1
# #                 continue

# #             # 计算 RT（无论如何都要，因为数据集需要 RT）
# #             R, T = calc_RT(mats, thks, nk_dict)
...
# #     # 报告
# #     print("\n✔ Done.")
# #     print(f"Generated: {len(df)} samples | Tried: {tried} | Dropped(except checks): {dropped}")
# #     print(f"Fail(struct): {fail_struct} | Fail(spec): {fail_spec}")
# #     print(f"Wavelength points: {len(WAVELENGTHS)} | Spectrum dim: {spec_mat.shape[1]} (=2*{len(WAVELENGTHS)})")
# #     print("\n== Tier distribution ==")
# #     print(df['pair_tier'].value_counts(normalize=True).sort_index())
# #     print("\nSaved to:", OUT_DIR)
# #     print("  - dbr_dataset.csv / dbr_dataset.pkl")
# #     print("  - total_structure.pkl / total_spectrum.pkl")


# # if __name__ == '__main__':
# #     generate_dataset()
# # dbr_dataset_generator_v4_verified_split.py
# # 在 v3 基础上：增加 total/train/dev 三套输出（total不划分，train/dev按比例二分）
# # 输出：
# # 1) OUT_DIR 下：
# #    - dbr_total_dataset.csv / .pkl
# #    - total_structure.pkl / total_spectrum.pkl
# # 2) DATASET_DIR 下（训练用）：
# #    - Structure_train.pkl / Spectrum_train.pkl
# #    - Structure_dev.pkl   / Spectrum_dev.pkl
# #
# # 训练阶段支持 spec_type='R'/'T'/'R_T'：
# # train_spectrum.pkl 每条为 [R..., T...] 拼接
