...
- overlay plots for quick visual inspection
...
# ==========================================================
# You can disable it if your environment is stable by setting ENABLE_TMM_FAST_PATCH=False.
...
# Choose more lambda0 points to increase peak coverage
...
# =========================
# 3) Validation thresholds (tune here)
# =========================
VAL_EPS = 0.02  # numeric tolerance

# flat curve detect
MIN_T_PTP = 0.01     # if T max-min < this -> reject
MIN_R_PTP = 0.01

# transmission filter constraint (optional but recommended)
REQUIRE_TMAX = True
TMAX_MIN = 0.60       # require max(T) >= 0.60

# peak not too close to band edges
EDGE_MARGIN_PTS = 5   # reject if argmax(T) within this margin

# end-point spike detect
END_SPIKE_K = 5
END_SPIKE_THRESH = 0.20  # if T[-1]-T[-k] > thresh -> reject
START_SPIKE_THRESH = 0.20  # if T[k]-T[0] > thresh -> reject
...
#    - This reduces "1.7 edge sudden rise" artifacts.
#    - Also enforce k>=0 and optionally a tiny floor (for stability).
# ==========================================================
K_FLOOR = 0.0  # keep physical (0) by default; if you want more damping use 1e-8/1e-6
...
def load_one_nk_csv(path: str, wavelengths_um: np.ndarray) -> np.ndarray:
    ...
    n_interp = interp1d(
        wl, n, bounds_error=False, fill_value=(n[0], n[-1])
    )
    k_interp = interp1d(
        wl, k, bounds_error=False, fill_value=(k[0], k[-1])
    )

    n_i = n_interp(wavelengths_um)
    k_i = k_interp(wavelengths_um)

    # re-clip to avoid negative drift
...
def gen_dbr_fp(nk_dict: Dict[str, torch.Tensor]) -> Tuple[List[str], List[int], dict]:
    ...
    # right: (L H)^pR  (approx symmetric around cavity)
    for i in range(pR * 2):
        m = L if (i % 2 == 0) else H
        mats.append(m)
        thks.append(qw_thk_nm(nk_dict[m], WAVELENGTHS_UM, lambda0_um))
...
def validate_curve(R: np.ndarray, T: np.ndarray) -> Tuple[bool, str, dict]:
    """
    Return: (is_valid, reason, extra_stats)
    """
    extra = {}
    ...
    S = R + T
    # energy: allow tiny numeric slack, but large >1 indicates instability
    ...
    # transmission peak requirement
    tmax = float(T.max())
    extra["tmax"] = tmax
    if REQUIRE_TMAX and (tmax < TMAX_MIN):
        return False, "tmax_low", extra

    # peak position not too close to edge
...
def main():
    ...
    while len(struct_list) < NUM_SAMPLES and tries < max_tries:
        ...
        for bi in range(cur):
            ...
            if not ok:
                ...
                # save a few bad examples for inspection
    ...
    # plots
    if overlay_T:
        plot_overlay(WAVELENGTHS_UM, overlay_T, "T",
                     f"Transmission overlay (N={len(overlay_T)})",
                     os.path.join(OUT_DIR, "overlay_T.png"))
        plot_overlay(WAVELENGTHS_UM, overlay_R, "R",
                     f"Reflection overlay (N={len(overlay_R)})",
                     os.path.join(OUT_DIR, "overlay_R.png"))
        print(f"[Plot] Saved: {os.path.join(OUT_DIR,'overlay_T.png')} and overlay_R.png")
